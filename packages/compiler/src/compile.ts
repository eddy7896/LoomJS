...
export function compile(snapshot: Snapshot): CompileResult {
  ...
  /**
   * The boundary module (L1), emitted unconditionally.
   *
   * The one thing the compiler writes that is not demand-driven, and deliberately: insurance
   * against the failure nobody predicted cannot be added once it happens.
   */
  files.push(emitBoundaryFile());
...
function emitApp(
  ...
  const routeFor = (route: RouteInfo, pad: number): string => {
    ...
    /**
     * One boundary per route (L1), inside the guard rather than outside it.
     *
     * Outside would mean a screen that throws while deciding whether you may see it gets caught
     * and shown — which is a redirect turned into an error message. Inside, the guard still
     * redirects and only the screen's own failure is caught.
     */
    const guarded = guardedElement(
      artboard,
      `<ErrorBoundary label={${JSON.stringify(`${artboard.name} did not load.`)}}><${route.componentName} /></ErrorBoundary>`,
      routes,
    );
    return `${' '.repeat(pad)}<Route path="${route.path}" element={${guarded}} />`;
  };
...
import { ErrorBoundary } from './state/boundary';
