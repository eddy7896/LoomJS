...
import { isDocument, isShell, pageOf, pageSize, shellDefinitions } from '@loom/compiler';
...
export function Canvas() {
  ...
  const isShellBoard = useCallback(
    (id: Id): boolean => {
      const definition = snapshot.definitions?.[id];
      return definition ? isShell(snapshot, definition) : false;
    },
    [snapshot],
  );
        ...
        {artboards.map((artboard) => (
          ...
          >
            <div className="artboard__label" onClick={() => setActiveArtboard(artboard.id)}>
              {artboard.name}
              {isShellBoard(artboard.id) ? <span className="chip">shell</span> : null}
